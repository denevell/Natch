...
public class ThreadsListModelImpl implements ThreadsListModel {
  ...
  public ThreadsAndNumTotalThreads list(String tag, int start, int maxNumPosts) {
    try {
      ...
      List<ThreadEntity> threads = mThreadModel.startTransaction().start(start)
          .max(maxNumPosts).namedQuery(listNamedQuery).list(ThreadEntity.class);
      if (threads == null)
        return null;
      long num = -1;
      num = mThreadModel.useTransaction(mThreadModel.getEntityManager())
          .namedQuery(countNamedQuery).count();
