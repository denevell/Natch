...
public class PostDeleteModelImpl implements PostDeleteModel {
  ...
  public int delete(long id, String username, boolean adminEditing) {
    try {
      final PostEntity pe = mPostModel.startTransaction().find(id, false,
          PostEntity.class);
      ...
      mThreadModel.useTransaction(postEntityManager).findAndUpdateOrDelete(
          pe.getThreadId(), new DeleteOrMerge<ThreadEntity>() {
            @Override
