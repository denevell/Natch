...
public class ThreadFromPostModelImpl implements ThreadFromPostModel {
  ...
  public ThreadEntity makeNewThread(long postId, String subject) {
    try {
      PostEntity post = mPostModel.startTransaction().find(postId, true,
          PostEntity.class);
      if (post == null)
        return null;
  ...
  private ThreadEntity createNewThreadFromPost(final PostEntity post) {
    ThreadEntity thread = mThreadModel.useTransaction(
        mPostModel.getEntityManager()).createOrUpdate(null,
